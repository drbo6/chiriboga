	...
	<head>
		...
		<script>
			...
			function ShowHackOpponentsModal() {
				...
				var modalHtml = '<div class="solo-menu" style="display: flex; flex-direction: column; align-items: center;">';
				modalHtml += '<div class="solo-logo" style="width: 100%;">';
				modalHtml += '<h1 class="logo-text" style="color: var(--crt-red); text-shadow: 0 0 5px var(--crt-red), 0 0 15px var(--glow-red), 0 0 35px var(--glow-red-dark); font-size: 28px;">HACK<br>OPPONENT</h1>';
				modalHtml += '</div>';
				...
				for (var i = 0; i < gauntletOpponents.length; i++) {
					...
					modalHtml += '<div class="hack-opponent-item' + defeatedClass + '" style="display: flex; flex-direction: column; align-items: center; ' + cursorStyle + '" onclick="ShowHackOptionsForOpponent(' + i + ');">';
			...
			function ShowHackOptionsForOpponent(opponentIndex) {
				...
				var hasPerk = opp.perk && opp.perk > 0;
				var isBossPerk = hasPerk && opp.perk >= 4;
				...
				modalHtml += '<h1 class="logo-text" style="color: var(--crt-red); text-shadow: 0 0 5px var(--crt-red), 0 0 15px var(--glow-red), 0 0 35px var(--glow-red-dark); margin: 20px 0;">' + oppName.toUpperCase() + '</h1>';
			...
			function AttemptHackPerk(opponentIndex) {
				...
				if (roll <= chance) {
					...
					SaveOpponentStateToURL();
					UpdateLaunchStrings();
					ShowHackResultModal(true, 'PERK REVEALED', 'You accessed their enhancement protocols.', opponentIndex);
				} else {
					// Failure
					SaveOpponentStateToURL();
					UpdateLaunchStrings();
			...
			function AttemptDisablePerk(opponentIndex) {
				var opp = gauntletOpponents[opponentIndex];
				var isBossPerk = opp.perk && opp.perk >= 4;
				...
				if (roll <= chance) {
					...
					SaveOpponentStateToURL();
					UpdateLaunchStrings();
					ShowHackResultModal(true, 'PERK DISABLED', 'You successfully disabled their enhancement. They will fight without their perk.', opponentIndex);
				} else {
					// Failure
					SaveOpponentStateToURL();
					UpdateLaunchStrings();
					ShowHackResultModal(false, 'HACK FAILED', 'Their security systems held. The chance has changed for your next attempt.', opponentIndex);
				}
			}
			...
			function ShowHackResultModal(success, title, message, opponentIndex) {
				...
				var modalHtml = '<div class="solo-menu" style="display: flex; flex-direction: column; align-items: center; width: 600px;">';
				modalHtml += '<h1 class="logo-text" style="color: ' + color + '; text-shadow: 0 0 5px ' + color + ', 0 0 15px ' + glowColor + ', 0 0 35px ' + glowDark + '; margin: 20px 0;">' + title + '</h1>';
			...
			function ShowPerkInfo(opponentIndex) {
				...
				var modalHtml = '<div class="solo-menu" style="display: flex; flex-direction: column; align-items: center; width: 600px;">';
				modalHtml += '<h1 class="logo-text" style="color: ' + titleColor + '; text-shadow: 0 0 5px ' + titleColor + ', 0 0 15px ' + titleGlow + ', 0 0 35px ' + titleGlowDark + '; margin: 20px 0;">' + perkName.toUpperCase() + '</h1>';
			...
			function ViewOpponentDecklist(opponentIndex) {
				...
				decklistHtml += '<h1 class="logo-text" style="color: var(--crt-green); text-shadow: 0 0 5px var(--crt-green), 0 0 15px var(--glow-green), 0 0 35px var(--glow-green-dark); margin: 20px 0;">' + oppName.toUpperCase() + '</h1>';
